...
This is Custodian's audit-log design in miniature: the same entry hash and
prev_hash, the same two checks. Custodian also writes every entry to object
storage under a compliance-mode lock, so not even an administrator can remove
it; in production this log would sit in S3 with Object Lock the same way. On
...
def verify_log(path: Path) -> dict:
    """Custodian's two checks: every entry hashes to what it claims, and every
