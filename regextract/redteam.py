...
This is the HR RAG assistant's method applied to extraction: a fixed set of
scripted attacks, run against the guarded pipeline and a bare one side by
side, so the guardrail's effect is a number rather than a claim. The attacks
...
from .extract import definitions_digest, extract_clause, parent_headings
