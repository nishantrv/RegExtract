...
from .extract import ClauseDraft
...
def _make_items_for_clause(
    *,
    ...
    groups = (
        ("obligation", primary.get("obligations") or []),
        ("entity", primary.get("entities") or []),
        ("cross_reference", primary.get("cross_references") or []),
    )
    ...
    for kind, payloads in groups:
        for payload in payloads:
            ...
            review = ReviewSignal(
                agreement=_agreement(kind, payload, secondary),
            ...
            items.append(
                Item(
                    ...
                    provenance=Provenance(
                        model=draft.metas[0].model if draft.metas else "",
                        ...
                        # provenance says so.
                        model_version=(draft.metas[0].served_model or draft.metas[0].model)
                        if draft.metas else "",
...
def find_conflicting_definitions(text: str) -> list[tuple[str, list[str]]]:
    ...
    for acronym in sorted(acronyms):
        ...
        trimmed = {_last_n_words(e, 4) for e in expansions if len(e.split()) >= 2}
        if len(trimmed) > 1:
