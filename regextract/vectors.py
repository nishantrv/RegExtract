...
Embeddings earn their place in exactly two jobs:
  ...
  1. Resolving an external reference to a document already in the corpus.
     ...
     Lives in retrieval.py: dense vectors fused with BM25, then a reranker,
     then a floor. Qdrant does the fusion natively when switched on.

  2. Matching clauses across revisions for stage 7. When revision 5 renumbers
     a clause, the diff has to recognise it as the same clause rather than a
     deletion plus an addition. Lives in publish.match_clauses.
