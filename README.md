...
> **The data in this repository is synthetic.** The original evaluation document (CARL-01) and its hand-labelled data are not included. `data/sample/SAMPLE-01.pdf` is a made-up regulation for trying the pipeline, and the files in `data/` and `gold/` are synthetic too (see `data/sample/README.md`). Tests that need CARL-01 are skipped automatically when `data/CARL-01.pdf` is absent. To reproduce the full evaluation, add the original document as `data/CARL-01.pdf` and replace the synthetic `data/` and `gold/` files with the originals.

**Quick start with the synthetic sample** (offline, no API key):
...
python run.py evaluate --pdf data/sample/SAMPLE-01.pdf # run the pipeline and score it
python run.py redteam  --pdf data/sample/SAMPLE-01.pdf # attack the guardrails, see what gets through
...
python run.py extract --pdf data/sample/SAMPLE-01.pdf --live --record
```
